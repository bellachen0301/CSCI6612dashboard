...
# df = default_df # 先设置了给后面用，这里需要改
# dash.register_page(__name__, name='3-Prediction', title='DH | 3-Prediction')
...
#                 #selected_buoy, 
#                 selected_column, order_p, order_d, order_q):
#     # 先默认用df 
#     # if selected_buoy:
#     #     df = pd.read_csv(f'data/{selected_buoy}.csv')
#     #     df['time'] = pd.to_datetime(df['time'])
